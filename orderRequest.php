...
// var_dump($imploded);
// $checkentry = mysqli_query($link, "SELECT * FROM amazon_orders WHERE amazonorderid = IN ({implode(',', $orderiddata)})");
// var_dump($checkentry);
// if(mysqli_num_rows($checkentry) == 0) {
$result = mysqli_query($link, "INSERT IGNORE into amazon_orders (amazonorderid, purchasedate, orderstatus, fulfillmentchannel, saleschannel, buyeremail, buyername, ordertype, latestshipdate, isbusinessorder, isprime, ispremiumorder) VALUES $imploded ON DUPLICATE KEY UPDATE amazonorderid=$orderid") or die(mysqli_error($link));
// }
